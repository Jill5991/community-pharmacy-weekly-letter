#!/usr/bin/env python3
"""
Community Pharmacy Weekly Letter Tracker
...
def cmd_scrape(days: int = 7):
    import asyncio, json
    console.print("\n[bold]Scraping configured web sources...[/bold]")
    results = asyncio.run(webscraper.fetch_all(days=days))
    for source, articles in results.items():
        console.print(f"  [cyan]{source}[/cyan]: {len(articles)} relevant articles found")
        for a in articles[:5]:
...
def cmd_journals():
    ...
    for journal, articles in results.items():
        console.print(f"  [cyan]{journal}[/cyan]: {len(articles)} relevant articles")
        for a in articles[:4]:
